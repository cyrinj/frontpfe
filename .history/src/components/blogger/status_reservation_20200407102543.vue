       ...
       <td>{{ item.trip.to }}</td>
        <td>{{ item.trip.theme }}</td>
          <td>{{ item.date_denvois}}</td>
            <td>{{ item. max_eff}}</td>
           <td width="10%" style="text-align:center">{{ item.status }}<br><button class="btn" @click="update(item)"><i class="fa fa-calendar-check"></i></button> <button class="btn1" @click="supprimer(item)"><i class="fa fa-trash"></i></button></td>
</tr>
      </tbody>
...
supprimer(item){
 ...
 if(r==true){
   console.log("cc",item)
          this.$store.dispatch('reservationdelete',item).then(data => {
           // console.log("aziz")
     })     
 ...
 console.log("i",i)
// console.log("ressssrow",this.rowData[i]);
  ...
  this.rowData.forEach(async(e) => {
     await  axios.post('http://localhost:3000/api/v2/host/allinfotrip',{id:e.tripid}).then(response => {
  ...
  console.log("tableau de demande de prise en charge",this.rowData);
            resolve(response.data.data)
        }).catch(err => reject(err))
