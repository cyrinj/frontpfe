...
<div id="status_reservation">
 <fieldset class="cadre_status ">
   <button @click="sh()">sh</button>
      ...
      <div v-for="(item,idx) in rowData" :key="idx" >
    <div  v-for="(item1,idx1) in item.trip" :key="idx1" >
  {{item1.title}}
    </div>
...
<!--  <td>{{ item.trip[0].country }}</td>
   <td>{{ item.trip[0].from }}
       <td>{{ item.trip[0].to }}</td>
        <td>{{ item.trip[0].theme }}</td>
          <td>{{ item[].dateenvois}}</td>
            <td>{{ item.host_nbrnumberparticipants}}</td>
           <td>{{ item.status }}</td>-->
</tr>
      </tbody>
...
sh(){
  console.log('trah',this.rowData[0].trip.title)
}
