       ...
       <td>{{ item.trip.to }}</td>
        <td>{{ item.trip.theme }}</td>
          <td>{{ item.date_denvois}}</td>
            <td>{{ item. max_eff}}</td>
           <td width="10%">{{ item.status }}<br><button class="btn" @click="update(item)"><i class="fa fa-calendar-check"></i></button> <button class="btn1" @click="supprimer(item)"><i class="fa fa-trash"></i></button></td>
</tr>
      </tbody>
