<template>
  <div id="model" class="x">
    <form-wizard
      ...
      color="#1E90FF"
    >
      <tab-content title="Primary informations" icon="ti-rocket" :before-change="validateAsync1">
      ...
      <tab-content title="Trip program" icon="ti-list" :before-change="validateAsync2">
      ...
      <tab-content title="Trip inspiration" icon="ti-wand" :before-change="onComplete">
        <div class="field">
...
export default {
  ...
  beforeDestroy() {
    if (
      (this.form.continent != null && this.form.continent != "") ||
      (this.form.country != null && this.form.country != "") ||
      (this.form.theme != null && this.form.theme != "")
    ) {
      this.updateDraft();
      console.log("dddd");
    }
  ...
  methods: {
    onComplete: function() {
      return new Promise((resolve, reject) => {
        if (
          (this.form.try1 == "alone" &&
            (this.form.inspiration == "" ||
              this.form.inspiration == null ||
              this.form.price == "" || this.form.price == null)) ||
          (this.form.try1 == "travel_agency" &&
            (this.form.agencyname == "" ||
              this.form.agencyname == null ||
              this.form.agencyemail == "" || this.form.agencyemail == null ||
              this.form.agencynumber == "" || this.form.agencynumber == null ||
              this.form.price == "" || this.form.price == null))
        ) {
          reject(
          ...
          if (this.form.draft == true) {
            ...
            this.$store.dispatch("tripdraftdelete", this.form).then(data => {
              // console.log("aziz")
            });
          } else {
            this.updateTrip();
        ...
        if (
          (this.form.bestperiode1 == "fromto" &&
            (this.form.continent == "" ||
              ...
              this.form.from == "" ||
              this.form.to == "")) ||
          (this.form.bestperiode1 == "months" &&
            (this.form.continent == "" ||
              this.form.country == "" ||
              this.form.theme == "" ||
              this.form.duration == "" ||
              this.form.month == "")) ||
          this.form.bestperiode1 == null
        ) {
          reject(
            "This is a custom validation error message. Click next again to get rid of the validation"
          );
    ...
    secondInputOptions() {
      if (this.form.continent === "Africa") {
        this.listcountry = this.Africa;
      }
      if (this.form.continent === "Europe") {
        this.listcountry = this.Europe;
      }
      if (this.form.continent === "Asia") {
        this.listcountry = this.Asia;
      }
      if (this.form.continent === "North_Amarica") {
        this.listcountry = this.North_Amarica;
      }
      if (this.form.continent === "South_Amarica") {
        this.listcountry = this.South_Amarica;
      }
    },
    updateTrip() {
